...
exports.Errors = function(props) {
  var _a = react_2.useState(true),
    trackToggled = _a[0],
    toggleTrack = _a[1];
  var errors = utility_1.RLFUtility.get.errors(props.tree);
  errors = utility_1.RLFUtility.get.error.positions(errors);
        ...
        .map(function(item) {
          ...
          item.errors.forEach(function(error) {
            var element = document.getElementById(error.flatKey),
              errorKey = utility_1.StringUtility.camelCaseToNormal(error.key),
              formattedKey = _.get(props.labels, error.flatKey) || errorKey;
